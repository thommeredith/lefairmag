...
class ESSBResourceBuilderSnippets {
	...
	public static function snippet_end() {
		return implode(" ", ESSBResourceBuilderSnippets::$snippet);
	}
	...
	public static function css_build_fanscounter_customizer() {
		...
		foreach ($network_list as $network => $title) {			
			...
			if ($color_isset != '') {
				...
				self::snippet_add('.essbfc-rc-'.$network.' { background-color: '.$color_isset.' !important; }');
				self::snippet_add('.essbfc-rcg-'.$network.' { background-color: '.$color_isset.' !important; }');
			}
		}
		return self::snippet_end();
	}
