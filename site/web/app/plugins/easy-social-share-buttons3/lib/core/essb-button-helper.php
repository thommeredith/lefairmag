...
class ESSBButtonHelper {
	...
	public static function draw_buttons_end($style = array(), $position = '', $salt = '', $like_or_share = 'share', $native_buttons = array(), $cached_counters = array()) {
		global $essb_options;
	...
	public static function draw_share_buttons($share = array(), $style = array(), $networks = array(), $networks_order = array(), $network_names = array(),
		...
		$sigle_counter_hidden_till = ESSBOptionValuesHelper::options_value($essb_options, 'button_counter_hidden_till');
		...
		$share['essb_encode_text_plus'] = ESSBOptionValuesHelper::options_bool_value($essb_options, 'essb_encode_text_plus');
		...
		foreach ( $networks_order as $single ) {
			...
			if (in_array ( $single, $networks )) {
				...
				if ($is_active_fullwidth && $count_displayed == 1) {
					$icon .= ' essb_item_fw_first';
				}

				if ($is_active_fullwidth && $count_displayed == 2) {
		...
		$content .= ESSBButtonHelper::draw_buttons_end ( $style, $position, $salt, $like_or_share, $native_buttons, $cached_counters );
	...
	public static function draw_share_counter_code($position, $counter, $counter_hidden) {
		...
		if ($counter_hidden != '') {
			if (parseInt($counter_hidden) > parseInt($counter)) {
