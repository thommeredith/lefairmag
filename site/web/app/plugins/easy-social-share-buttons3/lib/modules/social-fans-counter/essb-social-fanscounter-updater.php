...
class ESSBSocialFansCounterUpdater {
	...
	public static function flickr() {
		...
		$api_key = trim ( ESSBSocialFansCounterHelper::get_option ( 'flickr.api_key' ) );
		$username = trim ( ESSBSocialFansCounterHelper::get_option ( 'flickr.id' ) );
		...
		$result = 0;
		try {
			$data 	= self::essb_remote_get( "https://api.flickr.com/services/rest/?method=flickr.groups.getInfo&api_key=$api_key&group_id=$username&format=json&nojsoncallback=1");
