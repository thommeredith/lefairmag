...
if(isset($_GET['cjfm_reset']) && $_GET['cjfm_reset'] == $_GET['callback']){
	foreach ($options as $key => $value) {
		$wpdb->query("DELETE FROM $options_table WHERE option_name = '{$value['id']}'");
	}
	$location = cjfm_callback_url($_GET['callback']);
	wp_redirect( $location );
	exit;
}

// Reset section all
if(isset($_GET['cjfm_reset']) && $_GET['cjfm_reset'] == 'all'){
	foreach ($options as $key => $value) {
...
foreach ($options as $key => $option) {
	...
	if($option['type'] == 'heading'){
		...
		$settings_search_box = '
			<span class="settings-search-box">
				...
				<a href="'.cjfm_string(cjfm_callback_url($_GET['callback'])).'cjfm_reset='.$_GET['callback'].'" class="button-secondary cj-confirm margin-5-right margin-6-top inline-block" data-confirm="'.__("Are you sure?\nThis canont be undone.", 'cjfm').'">'.__('Reset Section', 'cjfm').'</a>
				<a href="'.cjfm_string(cjfm_callback_url($_GET['callback'])).'cjfm_reset=all" class="button-secondary cj-confirm margin-5-right margin-6-top inline-block" data-confirm="'.__("Are you sure?\nThis canont be undone.", 'cjfm').'">'.__('Reset All', 'cjfm').'</a>
	...
	if($option['type'] == 'text' || $option['type'] == 'textbox'){
		...
		$display[] = '<td class="cj-panel">';
		$display[] = '<input '.$params.' type="text" name="'.$option['id'].'" id="'.sanitize_title( $option['id'] ).'" value="'.esc_textarea(cjfm_get_option($option['id'])).'" /> <span class="cj-suffix">'.$suffix.'</span>';
		$display[] = '<div class="cj-info">'.__($option['info'], 'cjfm').'</div>';
		$display[] = '</td>';
		$display[] = '</tr>';
	}
	...
	if($option['type'] == 'multitextboxes'){
		$option_value = cjfm_get_option($option['id']);
		$display[] = '<tr class="searchable"><td class="cj-label"><label for="'.sanitize_title( $option['id'] ).'">'.__($option['label'], 'cjfm').'</label></td>';
		$display[] = '<td class="cj-panel">';
	...
	if($option['type'] == 'padding'){
		...
		$display[] = '<div class="alignleft margin-10-right padding-5-top"><label class="block textcenter padding-3-left">'.__('Top', 'cjfm').'</label><input style="width:100px; text-align:center;" placeholder="'.__('Top', 'cjfm').'" type="text" name="'.$option['id'].'[]" value="'.$top.'" /></div>';
		$display[] = '<div class="alignleft margin-10-right padding-5-top"><label class="block textcenter padding-3-left">'.__('Bottom', 'cjfm').'</label><input style="width:100px; text-align:center;" placeholder="'.__('Bottom', 'cjfm').'" type="text" name="'.$option['id'].'[]" value="'.$bottom.'" /></div>';
		$display[] = '<div class="alignleft margin-10-right padding-5-top"><label class="block textcenter padding-3-left">'.__('Left', 'cjfm').'</label><input style="width:100px; text-align:center;" placeholder="'.__('Left', 'cjfm').'" type="text" name="'.$option['id'].'[]" value="'.$left.'" /></div>';
		$display[] = '<div class="alignleft margin-10-right padding-5-top"><label class="block textcenter padding-3-left">'.__('Right', 'cjfm').'</label><input style="width:100px; text-align:center;" placeholder="'.__('Right', 'cjfm').'" type="text" name="'.$option['id'].'[]" value="'.$right.'" /></div>';
		$display[] = '</div>';
		$display[] = '<div class="cj-info">'.__($option['info'], 'cjfm').'</div>';
		$display[] = '</td>';
		$display[] = '</tr>';
	...
	if($option['type'] == 'pass' || $option['type'] == 'password'){
		...
		$display[] = '<tr class="searchable"><td class="cj-label"><label for="'.sanitize_title( $option['id'] ).'">'.__($option['label'], 'cjfm').'</label></td>';
		$display[] = '<td class="cj-panel">';
		$display[] = '<input '.$params.' type="password" name="'.$option['id'].'" id="'.sanitize_title( $option['id'] ).'" value="'.cjfm_get_option($option['id']).'" /> <span class="cj-suffix">'.$suffix.'</span>';
		$display[] = '<div class="cj-info">'.__($option['info'], 'cjfm').'</div>';
		$display[] = '</td>';
		$display[] = '</tr>';
	}
	...
	if($option['type'] == 'number'){
		...
		$display[] = '<tr class="searchable"><td class="cj-label"><label for="'.sanitize_title( $option['id'] ).'">'.__($option['label'], 'cjfm').'</label></td>';
		$display[] = '<td class="cj-panel">';
		$display[] = '<input '.$params.' type="number" name="'.$option['id'].'" id="'.sanitize_title( $option['id'] ).'" value="'.cjfm_get_option($option['id']).'" /> <span class="cj-suffix">'.$suffix.'</span>';
		$display[] = '<div class="cj-info">'.__($option['info'], 'cjfm').'</div>';
		$display[] = '</td>';
		$display[] = '</tr>';
	}
	...
	if($option['type'] == 'textarea'){
		$display[] = '<tr class="searchable"><td class="cj-label"><label for="'.sanitize_title( $option['id'] ).'">'.__($option['label'], 'cjfm').'</label></td>';
		$display[] = '<td class="cj-panel">';
		$display[] = '<textarea '.$params.' rows="5" cols="40" name="'.$option['id'].'" id="'.sanitize_title( $option['id'] ).'">'.cjfm_get_option($option['id']).'</textarea>';
		$display[] = '<div class="cj-info">'.__($option['info'], 'cjfm').'</div>';
		$display[] = '</td>';
		$display[] = '</tr>';
	}
	...
	if($option['type'] == 'textarea-readonly'){
		$display[] = '<tr class="searchable"><td class="cj-label"><label for="'.sanitize_title( $option['id'] ).'">'.__($option['label'], 'cjfm').'</label></td>';
		$display[] = '<td class="cj-panel">';
		$display[] = '<textarea '.$params.' rows="5" cols="40" name="'.$option['id'].'" readonly id="'.sanitize_title( $option['id'] ).'">'.cjfm_get_option($option['id']).'</textarea>';
		$display[] = '<div class="cj-info">'.__($option['info'], 'cjfm').'</div>';
		$display[] = '</td>';
		$display[] = '</tr>';
	}
	...
	if($option['type'] == 'code-css'){
		$display[] = '<tr class="searchable"><td class="cj-label"><label for="'.sanitize_title( $option['id'] ).'">'.__($option['label'], 'cjfm').'</label></td>';
		$display[] = '<td class="cj-panel">';
		$display[] = '<textarea '.$params.' class="cj-code-css" rows="5" cols="40" name="'.$option['id'].'" id="'.sanitize_title( $option['id'] ).'">'.cjfm_get_option($option['id']).'</textarea>';
		$display[] = '<div class="cj-info">'.__($option['info'], 'cjfm').'</div>';
		$display[] = '</td>';
		$display[] = '</tr>';
	}
	...
	if($option['type'] == 'code-js'){
		$display[] = '<tr class="searchable"><td class="cj-label"><label for="'.sanitize_title( $option['id'] ).'">'.__($option['label'], 'cjfm').'</label></td>';
		$display[] = '<td class="cj-panel">';
		$display[] = '<textarea '.$params.' class="cj-code-js" rows="5" cols="40" name="'.$option['id'].'" id="'.sanitize_title( $option['id'] ).'">'.cjfm_get_option($option['id']).'</textarea>';
		$display[] = '<div class="cj-info">'.__($option['info'], 'cjfm').'</div>';
		$display[] = '</td>';
		$display[] = '</tr>';
	}
	...
	if($option['type'] == 'multidropdown' || $option['type'] == 'multiselect'){
		$opts = null;
		$saved_opts = cjfm_get_option($option['id']);
	...
	if($option['type'] == 'taxonomies'){
		...
		$opts[] = '<option value="0">'.__('None', 'cjfm').'</option>';
		foreach ($paggs as $okey => $ovalue) {
			if(is_array(cjfm_get_option($option['id'])) && @in_array($ovalue->slug.'~~~~'.$ovalue->taxonomy, cjfm_get_option($option['id']))){
				$opts[] = '<option selected value="'.$ovalue->slug.'~~~~'.$ovalue->taxonomy.'">'.$ovalue->name.' ('.$ovalue->taxonomy.')</option>';
			}else{
				$opts[] = '<option value="'.$ovalue->slug.'~~~~'.$ovalue->taxonomy.'">'.$ovalue->name.' ('.$ovalue->taxonomy.')</option>';
			}
	...
	if($option['type'] == 'role'){
		...
		if(cjfm_get_option($option['id']) == 'non-user'){
			$opts[] = '<option selected value="non-user">'.__('Visitor', 'cjfm').'</option>';
		}else{
			$opts[] = '<option  value="non-user">'.__('Visitor', 'cjfm').'</option>';
		}
		$display[] = '<tr class="searchable"><td class="cj-label"><label for="'.sanitize_title( $option['id'] ).'">'.__($option['label'], 'cjfm').'</label></td>';
		$display[] = '<td class="cj-panel">';
		$display[] = '<select '.$params.' class="chzn-select-no-results" name="'.$option['id'].'" id="'.sanitize_title( $option['id'] ).'">';
	...
	if($option['type'] == 'date'){
		$display[] = '<tr class="searchable"><td class="cj-label"><label for="'.sanitize_title( $option['id'] ).'">'.__($option['label'], 'cjfm').'</label></td>';
		$display[] = '<td class="cj-panel">';
		$display[] = '<input '.$params.' type="text" name="'.$option['id'].'" id="'.sanitize_title( $option['id'] ).'" value="'.cjfm_get_option($option['id']).'" class="date" />';
		$display[] = '<div class="cj-info">'.__($option['info'], 'cjfm').'</div>';
		$display[] = '</td>';
		$display[] = '</tr>';
	}
	...
	if($option['type'] == 'color'){
		$saved_hex = cjfm_get_option($option['id']);
		$saved_color_hex = ($saved_hex == '') ? 'transparent' : $saved_hex;
		$display[] = '<tr class="searchable"><td class="cj-label"><label for="'.sanitize_title( $option['id'] ).'">'.__($option['label'], 'cjfm').'</label></td>';
		$display[] = '<td class="cj-panel">';
		$display[] = '<input '.$params.' type="text" name="'.$option['id'].'" id="'.sanitize_title( $option['id'] ).'" value="'.cjfm_get_option($option['id']).'" data-color="'.cjfm_get_option($option['id']).'" class="color-picker" /> <span class="color-hex"><code>'.$saved_color_hex.'</code></span>';
		$display[] = '<div class="cj-info">'.__($option['info'], 'cjfm').'</div>';
		$display[] = '</td>';
		$display[] = '</tr>';
	}
	...
	if($option['type'] == 'background'){
		...
		$opt_value = cjfm_get_option($option['id']);
	...
	if($option['type'] == 'submit'){
		...
		$display[] = '<input '.$params.' type="submit" id="'.$form_submit.'" name="'.$form_submit.'" value="'.__($option['label'], 'cjfm').'" class="button-primary" />'.$option['suffix'];
		$display[] = '</td>';
		$display[] = '</tr>';
	}
