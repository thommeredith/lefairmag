...
class PTB_Form_CPT {
	...
	public function add_settings_fields( $slug_admin_cpt ) {
		...
		add_settings_section(
			$this->settings_section_cpt,
			'',
			array( $this, 'cpt_section_cb' ),
			$this->slug_admin_cpt
		);
		...
		add_settings_section(
			$this->settings_section_mb,
			__( 'Meta Box Builder', 'ptb' ),
			array( $this, 'cmb_section_cb' ),
			$this->slug_admin_cpt
		);

		// Custom Labels section
		add_settings_section(
			$this->settings_section_cl,
			__( 'Custom Labels', 'ptb' ),
			array( $this, 'cl_section_cb' ),
			$this->slug_admin_cpt
		);
		...
		add_settings_section(
			$this->settings_section_ad,
			__( 'Advanced Options', 'ptb' ),
			array( $this, 'ad_section_cb' ),
			$this->slug_admin_cpt
		);
	...
	public function cmb_section_cb() {
		...
		printf(
			'<p>%s</p>',
			sprintf(__( ' Meta box allows users to enter additional meta data in the post. The fields you create here will appear in admin add/edit post page %s.', 'ptb'),
                                '<a href="http://themify.me/docs/post-type-builder-plugin-documentation#meta-box-builder">'.__('(learn more)','ptb').'</a>')
		);
		...
		foreach ( $cmb_types as $cmb_type_id => $cmb_type_args ) {
			...
			$html .= sprintf(
				'<a href="#" data-type="%s" id="ptb_cmb_%s" class="ptb_cmb_button">%s</a>',
				esc_attr( $cmb_type_id ),
				esc_attr( $cmb_type_id ),
				esc_html( $cmb_type_args['name'] )
			);

		}

		printf(
			'<div class="ptb_cmb_panel_wrapper"><div class="ptb_cmb_panel">%s%s</div>%s</div>',
			'<ul class="ptb_cmb_items_wrapper"></ul>',
			sprintf( '<div class="ptb_cmb_add_field"><span class="ti-plus circle"></span>%s</div>', __( 'Add Field', 'ptb' ) ),
			sprintf( '<div class="ptb_cmb_buttons_wrapper_wrapper"><div class="ptb_cmb_buttons_wrapper">%s</div></div>', $html )
		);
		...
		printf(
			...
			$this->get_input_id( $this->input_cmb_data ),
			$this->get_input_name( $this->input_cmb_data ),
			htmlentities( json_encode( $meta_box_data ) )
		);
		...
		foreach ( $cmb_types as $type => $args ) {
			...
			<li id="<?php print( $id ); ?>" data-template="<?php print( $type ); ?>"
				...
				<div class="ptb_cmb_item_body">
                                        <div class="ptb_cmb_input_row">
                                                <label for="<?php print( $id ); ?>_name" class="ptb_cmb_input_label">
                                                        <?php _e( "Name", 'ptb' ); ?>
                                                </label>
                                                        ...
                                                        <ul class="<?php echo $this->plugin_name ?>_language_fields">
                                                                <?php foreach ( $languages as $code => $lng ): ?>
                                                                        <li <?php if (isset( $lng['selected'] )): ?>class="<?php echo $this->plugin_name ?>_active_lng"<?php endif; ?>>
                                                                                <input type="text" id="<?php print( $id ); ?>_name_<?php echo $code ?>"/>
                                                                        </li>
                                                                <?php endforeach; ?>
                                                        </ul>
                                                </div>
                                        ...
                                        <div class="ptb_cmb_input_row">
                                                <label for="<?php print( $id ); ?>_description" class="ptb_cmb_input_label">
                                                ...
                                                <div class="ptb_cmb_input">
                                                        ...
                                                        <ul class="<?php echo $this->plugin_name ?>_language_fields">
                                                                <?php foreach ( $languages as $code => $lng ): ?>
                                                                        <li <?php if (isset( $lng['selected'] )): ?>class="<?php echo $this->plugin_name ?>_active_lng"<?php endif; ?>>
                                                                                <input type="text"
                                                                                       id="<?php print( $id ); ?>_description_<?php echo $code ?>"/>
                                                                        </li>
                                                                <?php endforeach; ?>
                                                        </ul>
                                                </div>
	...
	private function add_fields_main() {
		...
		add_settings_field(
			$this->get_field_id( PTB_Custom_Post_Type::SINGULAR_LABEL ),
			__( 'Singular Label', 'ptb' ),
			array( $this, 'post_type_singular_label_cb' ),
			$this->slug_admin_cpt,
			$this->settings_section_cpt,
			array( 'label_for' => $this->get_field_id( PTB_Custom_Post_Type::SINGULAR_LABEL ) )
		);

		add_settings_field(
			$this->get_field_id( PTB_Custom_Post_Type::PLURAL_LABEL ),
			__( 'Plural Label', 'ptb' ),
			array( $this, 'post_type_plural_label_cb' ),
			$this->slug_admin_cpt,
			$this->settings_section_cpt,
			array( 'label_for' => $this->get_field_id( PTB_Custom_Post_Type::PLURAL_LABEL ) )
		);

		add_settings_field(
			$this->get_field_id( PTB_Custom_Post_Type::SLUG ),
			__( 'Post Type Slug', 'ptb' ),
			array( $this, 'post_type_slug_cb' ),
			$this->slug_admin_cpt,
			$this->settings_section_cpt,
			array( 'label_for' => $this->get_field_id( PTB_Custom_Post_Type::SLUG ) )
		);

		add_settings_field(
			$this->get_field_id( PTB_Custom_Post_Type::DESCRIPTION ),
			__( 'Description', 'ptb' ),
			array( $this, 'post_type_description_cb' ),
			$this->slug_admin_cpt,
			$this->settings_section_cpt,
			array( 'label_for' => $this->get_field_id( PTB_Custom_Post_Type::DESCRIPTION ) )
		);

		

		add_settings_field(
			$this->get_field_id( PTB_Custom_Post_Type::SUPPORTS ),
			__( 'Supports', 'ptb' ),
			array( $this, 'post_type_supports_cb' ),
			$this->slug_admin_cpt,
			$this->settings_section_cpt,
			array( 'label_for' => $this->get_field_id( PTB_Custom_Post_Type::SUPPORTS ) )
		);

		add_settings_field(
			$this->get_field_id( PTB_Custom_Post_Type::TAXONOMIES ),
			__( 'Taxonomies', 'ptb' ),
			array( $this, 'post_type_tax_cb' ),
			$this->slug_admin_cpt,
			$this->settings_section_cpt,
			array( 'label_for' => $this->get_field_id( PTB_Custom_Post_Type::TAXONOMIES ) )
		);
	...
	private function add_fields_custom_labels() {
		...
		foreach ( $languages as $code => $lng ) {
			...
			$this->cpt->cl_menu_name[ $code ]          = isset($this->cpt->cl_menu_name[ $code ]) && $this->cpt->cl_menu_name[ $code ]?$this->cpt->cl_menu_name[ $code ]:__( '%s', 'ptb' );
                ...
                add_settings_field(
                        '',
                        sprintf(__('These labels are used in WordPress admin interface. Leave everything as default or change it to your custom labels %s.',  'ptb'),'<a href="https://codex.wordpress.org/Function_Reference/register_post_type" target="_blank">'.__('(learn more)','ptb').'</a>'),
                        array( $this, 'add_description_fields' ),
                        $this->slug_admin_cpt,
                        $this->settings_section_cl
		...
		foreach ( $fields as $key => $label ) {
			...
			add_settings_field(
				...
				$label,
				array( $this, 'add_fields_custom_labels_cb' ),
				$this->slug_admin_cpt,
				$this->settings_section_cl,
				array(
	...
	private function add_fields_advanced_options() {
                ...
                add_settings_field(
                        '',
                        sprintf(__('These are advanced post type options. Leave everything as default if you are not sure what they are %s.',  'ptb'),'<a href="https://codex.wordpress.org/Function_Reference/register_post_type#Arguments" target="_blank">'.__('(learn more)','ptb').'</a>'),
                        array( $this, 'add_description_fields' ),
                        $this->slug_admin_cpt,
                        $this->settings_section_ad
                ...
                add_settings_field(
			$this->get_field_id( PTB_Custom_Post_Type::IS_HIERARCHICAL ),
			__( 'Hierarchical', 'ptb' ),
			array( $this, 'post_type_is_hierarchical_cb' ),
			$this->slug_admin_cpt,
			$this->settings_section_ad,
			array( 'label_for' => $this->get_field_id( PTB_Custom_Post_Type::IS_HIERARCHICAL ) )
		);
                
		add_settings_field(
			$this->get_field_id( PTB_Custom_Post_Type::HAS_ARCHIVE ),
			__( 'Has Archive', 'ptb' ),
			array( $this, 'post_type_has_archive_cb' ),
			$this->slug_admin_cpt,
			$this->settings_section_ad,
			array( 'label_for' => $this->get_field_id( PTB_Custom_Post_Type::HAS_ARCHIVE ) )
		);
		...
		$fields = array(
			...
			add_settings_field(
				$this->get_field_id( $key ),
				$label,
				array( $this, 'add_fields_advanced_options_cb' ),
				$this->slug_admin_cpt,
				$this->settings_section_ad,
				array(
					'label_for' => $this->get_field_id( $key ),
	...
	public function post_type_singular_label_cb() {
		...
		print( $this->generate_input_text(
			PTB_Custom_Post_Type::SINGULAR_LABEL,
			$this->cpt->singular_label,
			false,
			__( '(eg. Movie)', 'ptb' )
		) );
	...
	public function post_type_plural_label_cb() {
		...
		print( $this->generate_input_text(
			PTB_Custom_Post_Type::PLURAL_LABEL,
			$this->cpt->plural_label,
			false,
			__( '(eg. Movies)', 'ptb' )
		) );
	...
	public function post_type_description_cb() {
		...
		print( $this->generate_input_text(
			PTB_Custom_Post_Type::DESCRIPTION,
			$this->cpt->description,
			false,
			__( "A short description of the post type", 'ptb' )
		) );
	...
	public function post_type_is_hierarchical_cb() {
		...
		print( $this->generate_input_radio_yes_no(
			PTB_Custom_Post_Type::IS_HIERARCHICAL,
			$this->cpt->is_hierarchical,
			__( 'Whether the post type is hierarchical (e.g. page). Allows parent to be specified.', 'ptb' )
		) );
	...
	public function post_type_has_archive_cb() {
		...
		print( $this->generate_input_radio_yes_no(
			PTB_Custom_Post_Type::HAS_ARCHIVE,
			$this->cpt->has_archive,
			__( 'Enables post type archive.', 'ptb' )
		) );
	...
	public function post_type_supports_cb() {
		...
		printf(
			'<fieldset class="clearfix"><div class="ptb-pull-left">%1$s</div><div class="ptb-pull-left">%2$s</div></fieldset>',
			implode( '', $arrays[0] ),
			implode( '', $arrays[1] )
		);
	...
	public function post_type_tax_cb() {
		...
		printf(
			'<fieldset class="clearfix"><div class="ptb-pull-left">%1$s</div><div class="ptb-pull-left">%2$s</div></fieldset>',
			implode( '', $arrays[0] ),
			isset( $arrays[1] ) && ! empty( $arrays[1] ) ? implode( '', $arrays[1] ) : ''
		);
                }
		printf(
			'<h2 style="margin-left: -4px;"><a target="_blank" href="?page=%s&action=add&ptype=%s" class="add-new-h2">%s</a></h2>',
			'ptb-ctx',
                        $this->id,
			__( 'Add Taxonomies', 'ptb' )
		);
                printf(__( 'Taxonomies are like sub categories and tags for custom post types %s.', 'ptb' ),
                           '<a href="https://codex.wordpress.org/Taxonomies" target="_blank">'.__('(learn more)','ptb').'</a>');
	...
	public function add_fields_custom_labels_cb( $args ) {
		...
		switch ( $referrer ) {
			...
			case PTB_Custom_Post_Type::CL_ADD_NEW :
				...
				print( $this->generate_input_text(
					PTB_Custom_Post_Type::CL_ADD_NEW,
					$this->cpt->cl_add_new,
					false,
					__( "The add new text. The default is Add New for both hierarchical and non-hierarchical types.", 'ptb' )
				) );
				break;
			...
			case PTB_Custom_Post_Type::CL_ADD_NEW_ITEM :
				...
				print( $this->generate_input_text(
					PTB_Custom_Post_Type::CL_ADD_NEW_ITEM,
					$this->cpt->cl_add_new_item,
					false,
					__( "The add new item text. Default is Add New Post/Add New Page.", 'ptb' )
				) );
				break;
			...
			case PTB_Custom_Post_Type::CL_EDIT_ITEM:
				...
				print( $this->generate_input_text(
					PTB_Custom_Post_Type::CL_EDIT_ITEM,
					$this->cpt->cl_edit_item,
					false,
					__( "The edit item text. Default is Edit Post/Edit Page.", 'ptb' )
				) );
				break;
			...
			case PTB_Custom_Post_Type::CL_NEW_ITEM:
				...
				print( $this->generate_input_text(
					PTB_Custom_Post_Type::CL_NEW_ITEM,
					$this->cpt->cl_new_item,
					false,
					__( "The view item text. Default is View Post/View Page.", 'ptb' )
				) );
				break;
			...
			case PTB_Custom_Post_Type::CL_ALL_ITEMS:
				...
				print( $this->generate_input_text(
					PTB_Custom_Post_Type::CL_ALL_ITEMS,
					$this->cpt->cl_all_items,
					false,
					__( "The all items text used in the menu. Default is the Name label.", 'ptb' )
				) );
				break;
			...
			case PTB_Custom_Post_Type::CL_VIEW_ITEM:
				...
				print( $this->generate_input_text(
					PTB_Custom_Post_Type::CL_VIEW_ITEM,
					$this->cpt->cl_view_item,
					false,
					__( "The view item text. Default is View Post/View Page.", 'ptb' )
				) );
				break;
			...
			case PTB_Custom_Post_Type::CL_SEARCH_ITEMS:
				...
				print( $this->generate_input_text(
					PTB_Custom_Post_Type::CL_SEARCH_ITEMS,
					$this->cpt->cl_search_items,
					false,
					__( "The search items text. Default is Search Posts/Search Pages.", 'ptb' )
				) );
				break;
			...
			case PTB_Custom_Post_Type::CL_NOT_FOUND:
				...
				print( $this->generate_input_text(
					PTB_Custom_Post_Type::CL_NOT_FOUND,
					$this->cpt->cl_not_found,
					false,
					__( "The not found text. Default is No posts found/No pages found.", 'ptb' )
				) );
				break;
			...
			case PTB_Custom_Post_Type::CL_NOT_FOUND_IN_TRASH:
				...
				print( $this->generate_input_text(
					PTB_Custom_Post_Type::CL_NOT_FOUND_IN_TRASH,
					$this->cpt->cl_not_found_in_trash,
					false,
					__( "The not found in trash text. Default is No posts found in Trash/No pages found in Trash.", 'ptb' )
				) );
				break;
			...
			case PTB_Custom_Post_Type::CL_PARENT_ITEM_COLON:
				...
				print( $this->generate_input_text(
					PTB_Custom_Post_Type::CL_PARENT_ITEM_COLON,
					$this->cpt->cl_parent_item_colon,
					false,
					__( "The parent text. This string isn't used on non-hierarchical types. In hierarchical ones the default is Parent Page.", 'ptb' )
				) );
				break;
			...
			case PTB_Custom_Post_Type::CL_MENU_NAME:
				...
				print( $this->generate_input_text(
					PTB_Custom_Post_Type::CL_MENU_NAME,
					$this->cpt->cl_menu_name,
					false,
					__( "This string is the name to give menu items.", 'ptb' )
				) );
				break;
		}
	...
	public function add_fields_advanced_options_cb( $args ) {
		...
		switch ( $referrer ) {
			...
			case PTB_Custom_Post_Type::AD_PUBLICLY_QUERYABLE :
				...
				print( $this->generate_input_radio_yes_no(
					PTB_Custom_Post_Type::AD_PUBLICLY_QUERYABLE,
					$this->cpt->ad_publicly_queryable,
					__( 'Whether post_type queries can be performed from the front end.', 'ptb' )
				) );
				break;
			...
			case PTB_Custom_Post_Type::AD_EXCLUDE_FROM_SEARCH :
				...
				print( $this->generate_input_radio_yes_no(
					PTB_Custom_Post_Type::AD_EXCLUDE_FROM_SEARCH,
					$this->cpt->ad_exclude_from_search,
					__( 'Whether to exclude posts with this post type from search results.', 'ptb' )
				) );
				break;
			...
			case PTB_Custom_Post_Type::AD_CAN_EXPORT :
				...
				print( $this->generate_input_radio_yes_no(
					PTB_Custom_Post_Type::AD_CAN_EXPORT,
					$this->cpt->ad_can_export,
					__( 'Can this post_type be exported.', 'ptb' )
				) );
				break;
			...
			case PTB_Custom_Post_Type::AD_SHOW_UI :
				...
				print( $this->generate_input_radio_yes_no(
					PTB_Custom_Post_Type::AD_SHOW_UI,
					$this->cpt->ad_show_ui,
					__( 'Whether to generate a default UI for managing this post type.', 'ptb' )
				) );
				break;
			...
			case PTB_Custom_Post_Type::AD_SHOW_IN_NAV_MENUS :
				...
				print( $this->generate_input_radio_yes_no(
					PTB_Custom_Post_Type::AD_SHOW_IN_NAV_MENUS,
					$this->cpt->ad_show_in_nav_menus,
					__( 'Include this post type in Appearance > Menus for selection.', 'ptb' )
				) );
				break;
			...
			case PTB_Custom_Post_Type::AD_SHOW_IN_MENU :
				...
				print( $this->generate_input_radio_yes_no(
					PTB_Custom_Post_Type::AD_SHOW_IN_MENU,
					$this->cpt->ad_show_in_menu,
					__( 'Show this post type in the WordPress admin menu.', 'ptb' )
				) );
				break;
			...
			case PTB_Custom_Post_Type::AD_MENU_POSITION :
				...
				printf( $this->generate_select(
					PTB_Custom_Post_Type::AD_MENU_POSITION,
					$this->get_menu_positions(),
					$this->cpt->ad_menu_position,
                                        __( 'The position to place this post type menu on admin sidebar.', 'ptb' )
				) );
				break;
			...
			case PTB_Custom_Post_Type::AD_MENU_ICON :
				...
				printf( $this->generate_select(
					PTB_Custom_Post_Type::AD_MENU_ICON,
					$this->get_dashicons(),
					$this->cpt->ad_menu_icon,
                                        sprintf(__('View %s',  'ptb'),'<a href="https://developer.wordpress.org/resource/dashicons/#search" target="_blank">'.__('all icons','ptb').'</a>')
				) );
				break;
			...
			case PTB_Custom_Post_Type::AD_CAPABILITY_TYPE :
				...
				printf( $this->generate_select(
					PTB_Custom_Post_Type::AD_CAPABILITY_TYPE,
					$capability_types,
					$this->cpt->ad_capability_type,
					__( 'The string to use to build the read, edit, and delete capabilities.', 'ptb' )
				) );
				break;
	...
	public function process_options( $input ) {
		...
		if ( ! empty( $this->id ) ) {
			...
			$message = sprintf(
				__( 'Custom post type "%1$s" successfully updated.', 'ptb' ),
				$this->cpt->singular_label[ $lang ]
			);
			...
			$message = sprintf(
				__( 'Custom post type "%1$s" successfully added.', 'ptb' ),
				$this->cpt->singular_label[ $lang ]
			);
	...
	private function get_input_name( $key ) {
		...
		$name = sprintf(
			'ptb_plugin_options[%s_post_type_%s]',
			$this->plugin_name,
			$key
		);
	...
	private function get_field_id( $field_key, $lng = false ) {
		...
		return ! $lng ?
			sprintf( '%s_%s_%s', $this->plugin_name, $this->key, $field_key )
			: sprintf( '%s_%s_%s_%s', $this->plugin_name, $this->key, $field_key, $lng );
	...
	private function get_field_name( $field_key, $lng = false ) {
		...
		return ! $lng ?
			sprintf( 'ptb_plugin_options[%s]', $this->get_field_id( $field_key, $lng ) )
			: sprintf( 'ptb_plugin_options[%s][%s]', $this->get_field_id( $field_key ), $lng );
	...
	private function generate_input_checkbox( $id, $value, $title, $checked ) {
		...
		$input = sprintf(
			'<label for="%1$s"><input type="checkbox" name="%2$s" id="%1$s" value="%3$s" %4$s /> %5$s</label><br>',
	...
	private function generate_input_text( $id, $value, $hidden = false, $description = '' ) {
		...
		if ( $hidden || in_array( $id, array( PTB_Custom_Post_Type::SLUG ) ) ) {
			...
			$input  = sprintf(
				'<input type="%1$s" %2$s id="%3$s" name="%4$s" value="%5$s" />',
				$hidden ? 'hidden' : 'text',
			...
			foreach ( $languages as $code => $lng ) {
				...
				$input .= sprintf(
					...
					'text',
					'class="regular-text"',
					esc_attr( $this->get_field_id( $id, $code ) ),
					esc_attr( $this->get_field_name( $id, $code ) ),
					esc_attr( $val )
				);
				$input .= '</li>';
			}
