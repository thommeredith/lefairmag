...
class PTB_Form_ImportExport {
	...
	public function add_settings_fields( $slug_admin_io ) {
		...
		add_settings_section(
			$this->settings_section_export,
			__( "Export", "ptb" ),
			array( $this, 'export_section_cb' ),
			$this->slug_admin_io
		);

		add_settings_section(
			$this->settings_section_import,
			__( "Import", "ptb" ),
			array( $this, 'import_section_cb' ),
			$this->slug_admin_io
		);
	...
	public function export_section_cb() {
		...
		<div class="ptb_interface ptb_export_wrapper">
			...
			<div class="ptb_tab_content_wrapper">
				<div id="ptb_export_cpt_list" class="ptb_tab_content">
					...
					foreach ( $cpt_collection as $cpt ) :
						$label = isset($cpt->plural_label[ $lng ])? $cpt->plural_label[ $lng ]:'';
						$slug  = $cpt->slug;
						$id    = "ptb_cpt_export_" . $slug;
						?>
						<label for="<?php print( $id ); ?>"><input type="checkbox" name="ptb_cpt_export[]"
						                                           id="<?php print( $id ); ?>"
						                                           value="<?php print( $slug ); ?>"> <?php print( $label ); ?>
						</label>
					<?php endforeach; ?>
				</div>
				...
				<div id="ptb_export_ctx_list" class="ptb_tab_content" style="display: none;">
					...
					foreach ( $ctx_collection as $ctx ) :
						$label = $ctx->plural_label[ $lng ];
						$slug  = $ctx->slug;
						$id    = "ptb_ctx_export_" . $slug;
						?>
						<label for="<?php print( $id ); ?>"><input type="checkbox" name="ptb_ctx_export[]"
						                                           id="<?php print( $id ); ?>"
						                                           value="<?php print( $slug ); ?>"> <?php print( $label ); ?>
						</label>
					<?php endforeach; ?>
				...
				<div id="ptb_export_ptt_list" class="ptb_tab_content" style="display: none;">
					...
					foreach ( $ptt_collection as $key => $ptt ) :
						$label = $ptt->get_name();
						$id    = "ptb_ctx_export_" . $key;
						?>
						<label for="<?php print( $id ); ?>"><input type="checkbox" name="ptb_ctx_export[]"
						                                           id="<?php print( $id ); ?>"
						                                           value="<?php print( $key ); ?>"> <?php print( $label ); ?>
						</label>
					<?php endforeach; ?>
				</div>
			...
			<form method="post" action="options.php" id="ptb_form_export">
				...
				<a href="#export" class="ptb_ie_button" id="ptb_export">Export</a>
			</form>
		...
		<div class="ptb_interface ptb_import_wrapper">
			...
			<form method="post" action="options.php" enctype="multipart/form-data" id="ptb_form_import">
				...
				<input type="hidden" name="ptb_plugin_options[ptb_ie_import]"/>
                                ...
                                <a href="#import" class="ptb_ie_button" id="ptb_import"><?php _e('Import',  $this->plugin_name)?></a>
			</form>
	...
	public function import( $input ) {
		...
		if ( array_key_exists( 'plugin', $data ) && $this->plugin_name == $data['plugin'] ) {
			...
			if ( array_key_exists( 'version', $data ) && $this->version == $data['version'] ) {
				...
				add_settings_error( $this->plugin_name . '_notices', '', __( "Imported data version not matching with current version of the plugin", "ptb" ), 'error' );
