...
class PTB_CMB_Textarea extends PTB_CMB_Base {
	 ...
	 */
	public function action_public_themplate( array $args, array $data, $meta_data, $lang = false ) {
            $text = nl2br($meta_data[$args['key']]);
            echo !has_shortcode($text,self::$plugin_name)?do_shortcode($text):$text;
	}

	public function render_post_type_meta( $post, $meta_box_id, $args ) {

		$meta_key = $meta_box_id;

		$value = get_post_meta( $post->ID, 'ptb_' . $meta_key, true );

		// this is a temporary code for migration to new version
		if ( empty( $value ) ) {
		...
		<textarea id="<?php print( esc_attr( $meta_key ) ); ?>" name="<?php print( esc_attr( $meta_key ) ); ?>" rows="5"
		          cols="40"><?php print( esc_attr( empty( $value ) ? PTB_Utils::get_label( $args['defaultValue'] ) : $value ) ); ?></textarea>
