...
class PTB_CMB_Image extends PTB_CMB_Base {
	...
	public function render_post_type_meta( $post, $meta_key, $args ) {
		...
		if ( ! empty( $value ) ) {
			$style = sprintf( 'style="background-image:url(%s)"', $value[1] );
		} else {
			$style = '';
		}
		...
		<div class="ptb_post_cmb_image_button_wrapper">
			<div class="ptb_post_cmb_image_wrapper">
				<a href="#" id="image_<?php print( esc_attr( $meta_key ) ); ?>"
				   class="ptb_post_cmb_image" <?php print( $style ); ?>>
					<span class="ti-plus"></span>
				</a>
			</div>
			<input type="hidden" name="<?php print( $name ); ?>"
			       value="<?php print( ! empty( $value ) ? esc_attr( $value[0] ) : '' ); ?>"/>
			<input type="text" name="<?php print( $name ); ?>"
			       value="<?php print( ! empty( $value ) ? esc_attr( $value[1] ) : '' ); ?>"/>
		</div>
	<?php
	...
	public function action_them_themplate( $id, $type, $args, $data = array(), array $languages = array() ) {
		...
		<div class="<?php echo $pluginame ?>_back_active_module_row">
			...
			<div class="<?php echo $pluginame ?>_back_active_module_input">
				<input id="<?php echo $pluginame ?>_<?php echo $id?>_width" type="text" class="<?php echo $pluginame ?>_xsmall"
				       name="[<?php echo $id ?>][width]"
				       <?php if (isset( $data['width'] )): ?>value="<?php echo $data['width'] ?>"<?php endif; ?> />
				<label for="<?php echo $pluginame ?>_<?php echo $id?>_width"><?php  _e( 'Width', 'ptb' ) ?></label>
				<input id="<?php echo $pluginame ?>_<?php echo $id?>_height" type="text" class="<?php echo $pluginame ?>_xsmall"
				       name="[<?php echo $id ?>][height]"
				       <?php if (isset( $data['height'] )): ?>value="<?php echo $data['height'] ?>"<?php endif; ?> />
				<label for="<?php echo $pluginame ?>_<?php echo $id?>_height"><?php  _e( 'Height', 'ptb' ) ?>(px)</label>
			</div>
		</div>
	<?php
	}
	...
	public function action_public_themplate( array $args, array $data, array $meta_data, $lang = false, $is_single = false ) {
		$pluginame = $this->get_plugin_name();
		?>
		<div class="<?php echo $pluginame ?>_image">
                        <?php if(isset($meta_data[0])):?>
                            <?php $url =  wp_get_attachment_url($meta_data[0]);?>
                        <?php elseif(isset($meta_data[1])):?>
                            <?php $url =  $meta_data[0];?>
                        <?php endif;?>
                        <?php $url = PTB_CMB_Base::ptb_resize($url,$data['width'],$data['height']);?>
                        <?php if($url):?>
                            <figure class="<?php echo $pluginame?>_post_image clearfix"><img src="<?php echo $url?>" /></figure>
                        <?php endif;?>
