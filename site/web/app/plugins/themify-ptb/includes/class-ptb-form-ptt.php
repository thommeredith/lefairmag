...
class PTB_Form_PTT {
	...
	public function main_section_cb() {
		...
		printf(
			'<input type="hidden" id="%1$s" name="%2$s[%1$s]" value="%3$s" />',
			$this->get_field_id( PTB_Post_Type_Template::ID ),
			$this->options_key,
			esc_attr( $this->get_edit_id() )
		);
	...
	private function add_fields() {
		...
		add_settings_field(
			$this->get_field_id( PTB_Post_Type_Template::NAME ),
			__( 'Template Name', 'ptb' ),
			array( $this, $this->get_callback_name( PTB_Post_Type_Template::NAME ) ),
			$this->slug_admin_ptt,
			$this->settings_section,
			array( 'label_for' => $this->get_field_id( PTB_Post_Type_Template::NAME ) )
		);

		add_settings_field(
			$this->get_field_id( PTB_Post_Type_Template::POST_TYPE ),
			__( 'Template for', 'ptb' ),
			array( $this, $this->get_callback_name( PTB_Post_Type_Template::POST_TYPE ) ),
			$this->slug_admin_ptt,
			$this->settings_section,
			array( 'label_for' => $this->get_field_id( PTB_Post_Type_Template::POST_TYPE ) )
		);
	...
	public function ptt_name_cb() {
		...
		printf(
			'<input required="required" type="text" class="regular-text" id="%1$s" name="%2$s[%1$s]" value="%3$s" />',
			esc_attr( $this->get_field_id( PTB_Post_Type_Template::NAME ) ),
			$this->options_key,
			esc_attr( $value )
		);
	...
	public function ptt_post_type_cb() {
                $post_create =  isset($_REQUEST['slug']) 
                                && $_REQUEST['slug']
                                && isset($_SERVER['HTTP_REFERER'] ) 
                                && $_SERVER['HTTP_REFERER']==admin_url('admin.php?page=ptb-cpt&action=add');//from page "post type add"
		...
		if ( $post_types ) {
			...
			foreach ( $post_types as $post_type ) {
				...
				$option  = sprintf(
					'<option %4$s value="%1$s" %2$s>%3$s</option>',
					esc_attr( $name ),
		...
		printf(
			'<select %5$s  id="%1$s" name="%2$s[%1$s]"><option value="">%4$s</option>%3$s</select>',
			esc_attr( $this->get_field_id( PTB_Post_Type_Template::POST_TYPE ) ),
			$this->options_key,
			implode( '', $options ),
			__( 'Select Post Type', 'ptb'),
			$disable
		);
                ?>
                <small class="<?php echo $this->plugin_name ?>_small_description">
                    <?php printf(__(' To avoid template errors, this template assignment select is not editable after the template is created. Only one template should be assigned per post type %s.', 'ptb'),'<a href="http://themify.me/docs/post-type-builder-plugin-documentation#templates" target="_blank">'.__('(learn more)','ptb').'</a>')?>
                </small>
                <?php
		if ( $update ) {
                        
			printf(
                            '<input type="hidden" value="%3$s" name="%2$s[%1$s]" />',
                            esc_attr( $this->get_field_id( PTB_Post_Type_Template::POST_TYPE ) ),
                            $this->options_key,
                            $value
                        );
