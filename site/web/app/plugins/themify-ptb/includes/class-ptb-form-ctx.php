...
class PTB_Form_CTX {
	...
	private function set_defaults() {
		...
		if ( isset( $_REQUEST['action'] ) ) {
			...
			if ( 'edit' == $_REQUEST['action'] && isset( $_REQUEST['ptb-ctx'] ) ) {

				$this->id = $_REQUEST['ptb-ctx'];
	...
	public function cpt_section_cb() {
		...
		print( $this->generate_input_text( PTB_Custom_Taxonomy::ID, $this->id, true ) );
	...
	private function add_fields_main() {
		...
		add_settings_field(
			$this->get_field_id( PTB_Custom_Taxonomy::SINGULAR_LABEL ),
			__( 'Singular Label', 'ptb' ),
			array( $this, 'ctx_singular_label_cb' ),
			$this->slug_admin_ctx,
			$this->settings_section_ctx,
			array( 'label_for' => $this->get_field_id( PTB_Custom_Taxonomy::SINGULAR_LABEL ) )
		);

		add_settings_field(
			$this->get_field_id( PTB_Custom_Taxonomy::PLURAL_LABEL ),
			__( 'Plural Label', 'ptb' ),
			array( $this, 'ctx_plural_label_cb' ),
			$this->slug_admin_ctx,
			$this->settings_section_ctx,
			array( 'label_for' => $this->get_field_id( PTB_Custom_Taxonomy::PLURAL_LABEL ) )
		);

		add_settings_field(
			$this->get_field_id( PTB_Custom_Taxonomy::SLUG ),
			__( 'Taxonomy Slug', 'ptb' ),
			array( $this, 'ctx_taxonomy_cb' ),
			$this->slug_admin_ctx,
			$this->settings_section_ctx,
			array( 'label_for' => $this->get_field_id( PTB_Custom_Taxonomy::SLUG ) )
		);

		add_settings_field(
			$this->get_field_id( PTB_Custom_Taxonomy::ATTACH_TO ),
			__( 'Attach to', 'ptb' ),
			array( $this, 'ctx_attach_to_cb' ),
			$this->slug_admin_ctx,
			$this->settings_section_ctx,
			array( 'label_for' => $this->get_field_id( PTB_Custom_Taxonomy::ATTACH_TO ) )
		);
	...
	public function ctx_taxonomy_cb() {
		...
		print( $this->generate_input_text(
			PTB_Custom_Taxonomy::SLUG,
			$this->ctx->slug,
			false,
			__( '(eg. producer. Should only contain lowercase english characters and the underscore or dash characters (no space or special characters)', 'ptb' )
		) );
	...
	public function ctx_singular_label_cb() {
		...
		print( $this->generate_input_text(
			PTB_Custom_Taxonomy::SINGULAR_LABEL,
			$this->ctx->singular_label,
			false,
			__( '(eg. Producer)', 'ptb' )
		) );
	...
	public function ctx_plural_label_cb() {
		...
		print( $this->generate_input_text(
			PTB_Custom_Taxonomy::PLURAL_LABEL,
			$this->ctx->plural_label,
			false,
			__( '(eg. Producers)', 'ptb' )
		) );
	...
	public function ctx_attach_to_cb() {
		...
		foreach ( $custom_post_types as $post_type ) {
			...
			$checkbox = $this->generate_input_checkbox(
				...
				$key,
				$post_type->label,
				isset($_GET['ptype']) && $_GET['ptype']==$key?true:$this->ctx->is_attached_to_post_type( $key ) || in_array( $post_type, $custom_post_types_by_tax )
			);
		...
		$checkbox_page = $this->generate_input_checkbox(
			...
			$key,
			__( 'Page', 'ptb' ),
			$this->ctx->is_attached_to_post_type( $key )
		);
		...
		printf(
			...
			implode( '', $arrays[1] )
                ...
                printf(__('Check which post type(s) to assign this taxonomy to %s.',  'ptb'),'<a href="https://codex.wordpress.org/Function_Reference/register_taxonomy" target="_blank">'.__('(learn more)','ptb').'</a>');
	}

	public function ctx_is_hierarchical_cb() {

		print( $this->generate_input_radio_yes_no(
			PTB_Custom_Taxonomy::IS_HIERARCHICAL,
			$this->ctx->is_hierarchical,
			__( 'Whether the taxonomy is hierarchical (e.g. producer). Allows parent to be specified.', 'ptb' )
		) );
	...
	private function add_fields_custom_labels() {
		...
		foreach ( $languages as $code => $lng ) {
			$this->ctx->cl_search_items[ $code ]               = isset($this->ctx->cl_search_items[ $code ]) && $this->ctx->cl_search_items[ $code ]?$this->ctx->cl_search_items[ $code ]:__( 'Search %s', 'ptb' ); 
			$this->ctx->cl_popular_items[ $code ]              = isset($this->ctx->cl_popular_items[ $code ]) && $this->ctx->cl_popular_items[ $code ]?$this->ctx->cl_popular_items[ $code ]:__( 'Popular %s', 'ptb' );
			$this->ctx->cl_all_items[ $code ]                  = isset($this->ctx->cl_all_items[ $code ]) && $this->ctx->cl_all_items[ $code ]?$this->ctx->cl_all_items[ $code ]:__( 'All %s','ptb' );
			$this->ctx->cl_parent_item[ $code ]                = isset($this->ctx->cl_parent_item[ $code ]) && $this->ctx->cl_parent_item[ $code ]?$this->ctx->cl_parent_item[ $code ]:__( 'Parent %s', 'ptb' );
			$this->ctx->cl_parent_item_colon[ $code ]          = isset($this->ctx->cl_parent_item_colon[ $code ]) && $this->ctx->cl_parent_item_colon[ $code ]?$this->ctx->cl_parent_item_colon[ $code ]:__( 'Parent %s:', 'ptb' );
			$this->ctx->cl_edit_item[ $code ]                  = isset($this->ctx->cl_edit_item[ $code ]) && $this->ctx->cl_edit_item[ $code ]?$this->ctx->cl_edit_item[ $code ]:__( 'Edit %s','ptb' ); 
			$this->ctx->cl_update_item[ $code ]                = isset($this->ctx->cl_update_item[ $code ]) && $this->ctx->cl_update_item[ $code ]?$this->ctx->cl_update_item[ $code ]:__( 'Update %s', 'ptb' ); 
			$this->ctx->cl_add_new_item[ $code ]               = isset($this->ctx->cl_add_new_item[ $code ]) && $this->ctx->cl_add_new_item[ $code ]?$this->ctx->cl_add_new_item[ $code ]:__( 'Add New %s', 'ptb' );
			$this->ctx->cl_new_item_name[ $code ]              = isset($this->ctx->cl_new_item_name[ $code ]) && $this->ctx->cl_new_item_name[ $code ]?$this->ctx->cl_new_item_name[ $code ]:__( 'New %s Name', 'ptb' );
			$this->ctx->cl_separate_items_with_commas[ $code ] = isset($this->ctx->cl_separate_items_with_commas[ $code ]) && $this->ctx->cl_separate_items_with_commas[ $code ]?$this->ctx->cl_separate_items_with_commas[ $code ]:__( 'Separate %s with commas','ptb' );
			$this->ctx->cl_add_or_remove_items[ $code ]        = isset($this->ctx->cl_add_or_remove_items[ $code ]) && $this->ctx->cl_add_or_remove_items[ $code ]?$this->ctx->cl_add_or_remove_items[ $code ]:__( 'Add or remove %s', 'ptb' );
			$this->ctx->cl_choose_from_most_used[ $code ]      = isset($this->ctx->cl_choose_from_most_used[ $code ]) && $this->ctx->cl_choose_from_most_used[ $code ]?$this->ctx->cl_choose_from_most_used[ $code ]:__( 'Choose from the most used %s', 'ptb' );
			$this->ctx->cl_menu_name[ $code ]                  = isset($this->ctx->cl_menu_name[ $code ]) && $this->ctx->cl_menu_name[ $code ]?$this->ctx->cl_menu_name[ $code ]:__( '%s', 'ptb' );
		}
	...
	public function add_fields_custom_labels_cb( $args ) {
		...
		switch ( $referrer ) {
			...
			case PTB_Custom_Taxonomy::CL_SEARCH_ITEMS :
				...
				print( $this->generate_input_text(
					PTB_Custom_Taxonomy::CL_SEARCH_ITEMS,
					$this->ctx->cl_search_items,
					false,
					__( "The search items text.", 'ptb' )
				) );
				break;
			...
			case PTB_Custom_Taxonomy::CL_POPULAR_ITEMS :
				...
				print( $this->generate_input_text(
					PTB_Custom_Taxonomy::CL_POPULAR_ITEMS,
					$this->ctx->cl_popular_items,
					false,
					__( "The popular items text. This string is not used on hierarchical taxonomies.", 'ptb' )
				) );
				break;
			...
			case PTB_Custom_Taxonomy::CL_ALL_ITEMS :
				...
				print( $this->generate_input_text(
					PTB_Custom_Taxonomy::CL_ALL_ITEMS,
					$this->ctx->cl_all_items,
					false,
					__( "The all items text.", 'ptb' )
				) );
				break;
			...
			case PTB_Custom_Taxonomy::CL_PARENT_ITEM :
				...
				print( $this->generate_input_text(
					PTB_Custom_Taxonomy::CL_PARENT_ITEM,
					$this->ctx->cl_parent_item,
					false,
					__( "The parent item text. This string is not used on non-hierarchical taxonomies such as post tags.", 'ptb' )
				) );
				break;
			...
			case PTB_Custom_Taxonomy::CL_PARENT_ITEM_COLON :
				...
				print( $this->generate_input_text(
					PTB_Custom_Taxonomy::CL_PARENT_ITEM_COLON,
					$this->ctx->cl_parent_item_colon,
					false,
					__( "The same as parent_item, but with colon : in the end.", 'ptb' )
				) );
				break;
			...
			case PTB_Custom_Taxonomy::CL_EDIT_ITEM :
				...
				print( $this->generate_input_text(
					PTB_Custom_Taxonomy::CL_EDIT_ITEM,
					$this->ctx->cl_edit_item,
					false,
					__( "The edit item text.", 'ptb' )
				) );
				break;
			...
			case PTB_Custom_Taxonomy::CL_UPDATE_ITEM :
				...
				print( $this->generate_input_text(
					PTB_Custom_Taxonomy::CL_UPDATE_ITEM,
					$this->ctx->cl_update_item,
					false,
					__( "The update item text.", 'ptb' )
				) );
				break;
			...
			case PTB_Custom_Taxonomy::CL_ADD_NEW_ITEM :
				...
				print( $this->generate_input_text(
					PTB_Custom_Taxonomy::CL_ADD_NEW_ITEM,
					$this->ctx->cl_add_new_item,
					false,
					__( "The add new item text.", 'ptb' )
				) );
				break;
			...
			case PTB_Custom_Taxonomy::CL_NEW_ITEM_NAME :
				...
				print( $this->generate_input_text(
					PTB_Custom_Taxonomy::CL_NEW_ITEM_NAME,
					$this->ctx->cl_new_item_name,
					false,
					__( "The new item name text.", 'ptb' )
				) );
				break;
			...
			case PTB_Custom_Taxonomy::CL_SEPARATE_ITEMS_WITH_COMMAS :
				...
				print( $this->generate_input_text(
					PTB_Custom_Taxonomy::CL_SEPARATE_ITEMS_WITH_COMMAS,
					$this->ctx->cl_separate_items_with_commas,
					false,
					__( "The separate item with commas text used in the taxonomy meta box. This string is not used on hierarchical taxonomies.", 'ptb' )
				) );
				break;
			...
			case PTB_Custom_Taxonomy::CL_ADD_OR_REMOVE_ITEMS :
				...
				print( $this->generate_input_text(
					PTB_Custom_Taxonomy::CL_ADD_OR_REMOVE_ITEMS,
					$this->ctx->cl_add_or_remove_items,
					false,
					__( "The add or remove items text and used in the meta box when JavaScript is disabled. This string is not used on hierarchical taxonomies.", 'ptb' )
				) );
				break;
			...
			case PTB_Custom_Taxonomy::CL_CHOOSE_FROM_MOST_USED :
				...
				print( $this->generate_input_text(
					PTB_Custom_Taxonomy::CL_CHOOSE_FROM_MOST_USED,
					$this->ctx->cl_choose_from_most_used,
					false,
					__( "The choose from most used text used in the taxonomy meta box. This string is not used on hierarchical taxonomies.", 'ptb' )
				) );
				break;
			...
			case PTB_Custom_Taxonomy::CL_MENU_NAME :
				...
				print( $this->generate_input_text(
					PTB_Custom_Taxonomy::CL_MENU_NAME,
					$this->ctx->cl_menu_name,
					false,
					__( "The menu name text. This string is the name to give menu items. If not set, defaults to value of name label.", 'ptb' )
				) );
				break;
	...
	private function add_fields_advanced_options() {
                ...
                add_settings_field(
			$this->get_field_id( PTB_Custom_Taxonomy::IS_HIERARCHICAL ),
			__( 'Hierarchical', 'ptb' ),
			array( $this, 'ctx_is_hierarchical_cb' ),
			$this->slug_admin_ctx,
			$this->settings_section_ad,
			array( 'label_for' => $this->get_field_id( PTB_Custom_Taxonomy::IS_HIERARCHICAL ) )
		);
	...
	public function add_fields_advanced_options_cb( $args ) {
		...
		switch ( $referrer ) {
			...
			case PTB_Custom_Taxonomy::AD_PUBLICLY_QUERYABLE :
				...
				print( $this->generate_input_radio_yes_no(
					PTB_Custom_Taxonomy::AD_PUBLICLY_QUERYABLE,
					$this->ctx->ad_publicly_queryable,
					__( 'If the taxonomy should be publicly queryable.', 'ptb' )
				) );
				break;
			...
			case PTB_Custom_Taxonomy::AD_SHOW_UI :
				...
				print( $this->generate_input_radio_yes_no(
					PTB_Custom_Taxonomy::AD_SHOW_UI,
					$this->ctx->ad_show_ui,
					__( 'Whether to generate a default UI for managing this taxonomy.', 'ptb' )
				) );
				break;
			...
			case PTB_Custom_Taxonomy::AD_SHOW_TAG_CLOUD :
				...
				print( $this->generate_input_radio_yes_no(
					PTB_Custom_Taxonomy::AD_SHOW_TAG_CLOUD,
					$this->ctx->ad_show_tag_cloud,
					__( 'Whether to allow the Tag Cloud widget to use this taxonomy.', 'ptb' )
				) );
				break;
			...
			case PTB_Custom_Taxonomy::AD_SHOW_ADMIN_COLUMN :
				...
				print( $this->generate_input_radio_yes_no(
					PTB_Custom_Taxonomy::AD_SHOW_ADMIN_COLUMN,
					$this->ctx->ad_show_admin_column,
					__( 'Whether to allow automatic creation of taxonomy columns on associated post-types table.', 'ptb' )
				) );
				break;
	...
	public function process_options( $input ) {
		...
		if ( ! empty( $this->id ) ) {
			...
			$message = sprintf(
				__( 'Custom taxonomy "%1$s" successfully updated.', 'ptb' ),
				$this->ctx->singular_label[ $lng ]
			);
			...
			$message = sprintf(
				__( 'Custom taxonomy "%1$s" successfully added.', 'ptb' ),
				$this->ctx->singular_label[ $lng ]
			);
