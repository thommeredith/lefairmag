...
class PTB_List_Table_PTT extends WP_List_Table {
	...
	function column_name( $item ) {
		...
		$actions = array(
			...
			'delete' => sprintf( '<a href="?page=%s&action=%s&ptb-ptt=%s">Delete</a>', $_REQUEST['page'], 'delete', $item['ID'] ),
		);

		//Return the title contents
		return sprintf( '%1$s %2$s',
			/*$1%s*/
			$item[ PTB_Post_Type_Template::NAME ],
			/*$3%s*/
	...
	function process_bulk_action() {
		...
		if ( 'delete' === $this->current_action() ) {
			if ( isset( $_REQUEST[ $this->_args['singular'] ] ) ) {
				...
				$t_ids = $_REQUEST[ $this->_args['singular'] ];
				$t_ids = array_values( $t_ids );
                                $names = array();
				...
				if ( $count > 0 ) {
					...
					if ( $count === 1 ) {
						...
						$message = sprintf(
							__( '%1$s templates successfully removed.', 'ptb'),
							implode( ', ', $names )
						);
					}
