...
class PTB_Custom_Taxonomy {
	...
	public function deserialize( $source ) {
		...
		if ( array_key_exists( self::ATTACH_TO, $source ) ) {
		...
		if ( array_key_exists( self::CL_POPULAR_ITEMS, $source ) ) {
		...
		if ( array_key_exists( self::CL_PARENT_ITEM, $source ) ) {
		...
		if ( array_key_exists( self::CL_UPDATE_ITEM, $source ) ) {
		...
		if ( array_key_exists( self::CL_NEW_ITEM_NAME, $source ) ) {
			$this->cl_new_item_name = $source[ self::CL_NEW_ITEM_NAME ];
		}

		if ( array_key_exists( self::CL_SEPARATE_ITEMS_WITH_COMMAS, $source ) ) {
			$this->cl_separate_items_with_commas = $source[ self::CL_SEPARATE_ITEMS_WITH_COMMAS ];
		}

		if ( array_key_exists( self::CL_ADD_OR_REMOVE_ITEMS, $source ) ) {
			$this->cl_add_or_remove_items = $source[ self::CL_ADD_OR_REMOVE_ITEMS ];
		}

		if ( array_key_exists( self::CL_CHOOSE_FROM_MOST_USED, $source ) ) {
		...
		if ( array_key_exists( self::AD_SHOW_TAG_CLOUD, $source ) ) {
			$this->ad_show_tag_cloud = $source[ self::AD_SHOW_TAG_CLOUD ];
		}

		if ( array_key_exists( self::AD_SHOW_ADMIN_COLUMN, $source ) ) {
