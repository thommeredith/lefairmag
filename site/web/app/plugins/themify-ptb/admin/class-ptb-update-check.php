...
class PTB_Update_Check {
	...
	public function prompt() {
		...
		<div class="prompt-box">
			...
			<form id="themify_update_form" method="post" action="<?php echo admin_url( 'admin.php?page=ptb-cpt&action=upgrade&login=true' ); ?>">
	...
	public function check_version() {
		...
		if ( is_object( $newUpdate ) && $time_not_changed ) {
			if ( version_compare( $this->version, $newUpdate->version, '<') ) {
				$notifications .= sprintf( __('<p class="update update-nag %s">%s version %s is now available. <a href="%s" title="" class="%s" target="%s" data-plugin="%s" data-package_url="%s" data-nicename_short="%s" data-update_type="%s">Update now</a> or view the <a href="%s" title="" class="themify_changelogs" target="_blank" data-changelog="%s">changelog</a> for details.</p>', 'ptb'),
	...
	public function enqueue() {
		wp_enqueue_script( 'themify-ptb-upgrader', plugin_dir_url( __FILE__ ) . '/js/ptb-upgrader.js', array('jquery'), false, true );
	}
	...
	function themify_ptb_updater(){
		...
		if( ! $url || ! $plugin_slug  || !$name) return;
