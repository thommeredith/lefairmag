	...
	function showLogin(status){
		...
		if(status == 'error'){
			if($('.prompt-box .prompt-error').length == 0){
				$('.prompt-box .prompt-msg').after('<p class="prompt-error">' + ptb_js.invalid_login + '</p>');
			}
		} else {
				...
				if(username != "" && password != ""){
					...
					$.post(
						...
						function(data){
							...
							if(data == 'true'){
								hideAlert();
								$('#themify_update_form').append( '<input type="hidden" name="plugin" value="'+ _updater_el.data( 'plugin' ) +'" /><input type="hidden" name="package_url" value="'+ _updater_el.data( 'package_url' ) +'" /><input type="hidden" name="nicename_short" value="'+ _updater_el.data( 'nicename_short' ) +'" />' ).submit();
							} else{
								hideAlert('error');	
								showLogin('error');
							}
