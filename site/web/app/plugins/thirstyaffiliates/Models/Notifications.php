...
class Notifications implements Model_Interface {
  ...
  public function get() {
    ...
    if ( ! empty( $notifications['active'] ) ) {
      foreach ( $notifications['active'] as $key => $notification ) {
        if ( strtotime( $notification['original_start'] . ' America/New_York' ) > strtotime( date('F j, Y') . ' America/New_York' ) ) {
          unset( $notifications['active'][ $key ] );
        }
      }
    }
  ...
  public function get_notifications_with_human_readeable_start_time( $notifications ) {
    ...
    foreach ( $notifications as $key => $notification ) {
      ...
      $notifications[ $key ]['original_start'] = $notification['start'];
    }
