...
class Bootstrap implements Model_Interface {
    ...
    */
    ...
    public function run() {
        ...
        add_action( 'activated_plugin' , array( $this , 'maybe_activate_onboarding_redirect' ) );
    }

    public function maybe_activate_onboarding_redirect($plugin) {

        if($plugin != $this->_constants->_PLUGIN_BASENAME) {
          return;
        }

        if(!is_user_logged_in() || wp_doing_ajax() || !is_admin() || is_network_admin() || !$this->_helper_functions->is_admin()) {
          return;
        }

        if($this->_helper_functions->is_post_request() && (isset($_POST['action']) || isset($_POST['action2']))) {
        ...
        $onboarded = $wpdb->get_var("SELECT option_value FROM {$wpdb->options} WHERE option_name = 'ta_onboarded'");

        if($onboarded === null) {
          nocache_headers();
          wp_redirect(admin_url('options.php?page=thirstyaffiliates_onboarding'), 307);
          exit;
        }
    }
